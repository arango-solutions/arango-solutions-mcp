...
from arango.exceptions import AnalyzerCreateError, AnalyzerDeleteError, AnalyzerGetError, AnalyzerListError, ArangoServerError
...
class AnalyzerManagementAgent(ArangoAgentBase):
    ...
    async def arun(self, mcp_tool_inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        properties: Optional[Dict[str, Any]] = mcp_tool_inputs.get("properties") # Can be Dict or str (for N-Gram)
        features: Optional[List[str]] = mcp_tool_inputs.get("features")

        logger.info(f"AnalyzerManagementAgent: Op='{operation}', DB='{database_name}', Analyzer='{analyzer_name}'")
        ...
        try:
            ...
            if operation == "list_analyzers":
                ...
                if analyzer_type == "ngram" and isinstance(properties, dict) and \
                   ("minN" not in properties or "maxN" not in properties):
                ...
                success = db.delete_analyzer(analyzer_name, ignore_missing=True) # Set ignore_missing based on desired behavior
        ...
        except (AnalyzerListError, AnalyzerCreateError, AnalyzerDeleteError, AnalyzerGetError, ArangoServerError) as e:
            logger.error(f"AnalyzerManagementAgent: ArangoDB error - {e}")
            return {"error": f"ArangoDB Analyzer Error: {e.error_message if hasattr(e, 'error_message') else str(e)}"}
