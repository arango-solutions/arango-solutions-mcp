...
class AQLExecutionAgent(ArangoAgentBase):
    ...
    async def arun(self, mcp_tool_inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"AQLExecutionAgent: Executing AQL in DB '{database_name}': {aql_query[:100]}... with bind_vars: {bind_vars}")
        ...
        try:
            ...
            cursor = db_to_query.aql.execute(aql_query, bind_vars=bind_vars, count=True, full_count=True)
            ...
            response = {
                ...
                "full_count": cursor.full_count() if hasattr(cursor, 'full_count') else None, # Total documents matching (if applicable)
                "results": results,
            ...
            return {"error": f"AQL Execution Error: {e.error_message}", "error_code": e.error_code, "details": str(e)}
            ...
            return {"error": f"ArangoDB Server Error: {e.error_message}", "error_code": e.error_code, "details": str(e)}
        except Exception as e:
            logger.error(f"AQLExecutionAgent: Unexpected error during AQL execution in DB '{database_name}': {e}", exc_info=True)
