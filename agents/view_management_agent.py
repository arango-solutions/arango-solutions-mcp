...
from arango.exceptions import (
    ViewCreateError, ViewDeleteError, ViewGetError, ViewListError,
    ViewUpdateError, ArangoServerError
)
...
class ViewManagementAgent(ArangoAgentBase):
    ...
    def _view_exists(self, db_instance, view_name: str) -> bool:
        ...
        try:
            ...
            if e.error_code in [1203, 1207]:
                logger.debug(f"_view_exists: View '{view_name}' not found (ViewGetError code {e.error_code}).")
                return False
            # For other ViewGetErrors (e.g., permission issues), re-raise
            logger.warning(f"_view_exists: Unexpected ViewGetError for view '{view_name}' (code: {e.error_code}): {e.error_message}", exc_info=True)
            raise
        except ArangoServerError as e:
            # Some ArangoDB versions might throw a more generic ArangoServerError for not found
            if e.error_code in [1203, 1207]:
                logger.debug(f"_view_exists: View '{view_name}' not found (ArangoServerError code {e.error_code}).")
                return False
            logger.warning(f"_view_exists: Unexpected ArangoServerError for view '{view_name}' (code: {e.error_code}): {e.error_message}", exc_info=True)
            raise
        except Exception as e: # Catch any other unexpected exceptions
            logger.error(f"_view_exists: Unexpected exception while checking view '{view_name}': {e}", exc_info=True)
            raise
    ...
    async def arun(self, mcp_tool_inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        properties: Dict[str, Any] = mcp_tool_inputs.get("properties") if mcp_tool_inputs.get("properties") is not None else {}

        logger.info(f"ViewManagementAgent: Op='{operation}', DB='{database_name}', View='{view_name}', Type='{view_type}', Props='{str(properties)[:100]}...'")
        ...
        try:
            ...
            if operation == "list_views":
                ...
                if not view_name or not view_type:
                    logger.warning("ViewManagementAgent: View name and type are required for create_view.")
                ...
                if self._view_exists(db, view_name): # CHECK 1
                    return {"status": f"View '{view_name}' already exists in database '{database_name}'."}
                ...
                logger.info(f"Attempting to create view '{view_name}' of type '{view_type}' with properties: {current_properties}")
                
                if view_type.lower() == "arangosearch":
                    view_info = db.create_arangosearch_view(name=view_name, properties=current_properties)
                elif view_type.lower() == "search-alias":
                    if not current_properties or "indexes" not in current_properties:
                        return {"error": "For 'search-alias' view type, 'properties' argument must include 'indexes' definition."}
                    view_info = db.create_view(name=view_name, view_type="search-alias", properties=current_properties)
                else:
                    return {"error": f"Unsupported view type '{view_type}'. Supported types: 'arangosearch', 'search-alias'."}
            ...
            # and handle the boolean return appropriately.
                ...
                logger.info(f"Updating view '{view_name}' of type '{view_actual_type}' with properties: {properties}")

                if view_actual_type == 'arangosearch':
                     updated_props = db.update_arangosearch_view(name=view_name, properties=properties)
                elif view_actual_type == 'search-alias': 
                     updated_props = db.update_view(name=view_name, properties=properties) 
                else: 
                     logger.warning(f"Attempting generic update for unknown or non-specific view type: {view_actual_type}")
                     updated_props = db.update_view(name=view_name, properties=properties)
                ...
                view_obj_props = db.view(view_name)
                view_actual_type = view_obj_props.get('type')
                logger.info(f"Replacing properties for view '{view_name}' of type '{view_actual_type}' with: {properties}")

                if view_actual_type == 'arangosearch':
                    replaced_props = db.replace_arangosearch_view(name=view_name, properties=properties)
                elif view_actual_type == 'search-alias':
                    if "indexes" not in properties: # Ensure properties are valid for search-alias replacement
                         return {"error": "For replacing 'search-alias' view type, 'properties' must include 'indexes' definition."}
                    replaced_props = db.replace_view(name=view_name, view_type="search-alias", properties=properties)
                else:
                    logger.warning(f"Attempting generic replace for non-specific view type: {view_actual_type}.")
                    replaced_props = db.replace_view(name=view_name, view_type=view_actual_type, properties=properties)

                return {"status": "View properties replaced.", "replaced_properties": replaced_props}
        ...
        except (ViewListError, ViewCreateError, ViewDeleteError, ViewGetError, ViewUpdateError, ArangoServerError) as e:
            logger.error(f"ViewManagementAgent: ArangoDB error - Op: {operation}, DB: {database_name}, View: {view_name}, Error: {e}", exc_info=True)
            return {"error": f"ArangoDB View Error: {getattr(e, 'error_message', str(e))}", "error_code": getattr(e, 'error_code', None)}
        # Catch any other unexpected exceptions
        except Exception as e:
            logger.error(f"ViewManagementAgent: Unexpected error - Op: {operation}, DB: {database_name}, View: {view_name}, Error: {e}", exc_info=True)
