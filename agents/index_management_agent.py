...
from arango.exceptions import IndexCreateError, IndexDeleteError, IndexListError, ArangoServerError
...
class IndexManagementAgent(ArangoAgentBase):
    ...
    async def arun(self, mcp_tool_inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"IndexManagementAgent: Op='{operation}', DB='{database_name}', Collection='{collection_name}'")
        ...
        try:
            ...
            if operation == "list_indexes":
                ...
                if index_type == "persistent":
                    ...
                    if not fields:
                        return {"error": "Geo index requires 'fields' to be specified."}
                    index_info = collection.add_geo_index(
                        fields=fields,
                        geo_json=index_definition.get("geoJson", False),
                        name=index_definition.get("name")
                    )
                elif index_type == "ttl":
                    if not fields:
                        return {"error": "TTL index requires 'fields' to be specified."}
                    index_info = collection.add_ttl_index(
                        fields=fields,
                        expiry_time=index_definition.get("expireAfterSeconds", 0),
                        name=index_definition.get("name")
                    )
                elif index_type == "fulltext":
                    if not fields:
                        return {"error": "Fulltext index requires 'fields' to be specified."}
                    index_info = collection.add_fulltext_index(
                        fields=fields,
                        min_length=index_definition.get("minLength", 2),
                        name=index_definition.get("name")
                    )
                else:
                    return {"error": f"Unsupported index type: {index_type}. Supported types: persistent, geo, ttl, fulltext"}
                ...
                indexes = collection.indexes()
                primary_index_id = next((idx['id'] for idx in indexes if idx['type'] == 'primary'), None)
                target_index_id = index_id_or_name
                if index_id_or_name not in [idx['id'] for idx in indexes]: # if name is given, try to find id
                    target_index_obj = next((idx for idx in indexes if idx.get('name') == index_id_or_name), None)
                    if target_index_obj:
                        target_index_id = target_index_obj['id']
                    else: # not found by id or name
                         return {"error": f"Index '{index_id_or_name}' not found in collection '{collection_name}'."}


                if primary_index_id and target_index_id == primary_index_id:
                    return {"error": "The primary index cannot be deleted."}

                success = collection.delete_index(target_index_id, ignore_missing=False) # already checked existence
                return {"status": f"Index '{index_id_or_name}' deleted successfully.", "success": success}
            
            else:
                return {"error": f"Unknown index operation: {operation}"}

        except (IndexListError, IndexCreateError, IndexDeleteError, ArangoServerError) as e:
            logger.error(f"IndexManagementAgent: ArangoDB error - {e}")
            return {"error": f"ArangoDB Index Error: {e.error_message if hasattr(e, 'error_message') else str(e)}"}
