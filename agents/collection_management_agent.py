...
from arango.exceptions import (
    CollectionCreateError, CollectionDeleteError, CollectionListError,
    CollectionPropertiesError, CollectionConfigureError, ArangoServerError
)
...
class CollectionManagementAgent(ArangoAgentBase):
    ...
    async def arun(self, mcp_tool_inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"CollectionManagementAgent: Op='{operation}', DB='{database_name}', Collection='{collection_name}'")
        ...
        try:
            ...
            if operation == "list_collections":
                ...
                user_collections_info = [
                    col_info for col_info in all_collections_info 
                    if not col_info.get('name', '').startswith('_')
                ]
                if not user_collections_info:
                    return {"database_name": database_name, "collections": [], "message": f"No user-defined collections found in database '{database_name}'."}
                return {"database_name": database_name, "collections": user_collections_info}
            
            if operation in ["create_collection", "delete_collection", "get_collection_properties"] and not collection_name:
                return {"error": f"Collection name is required for operation '{operation}'."}

            if operation == "create_collection":
                if db.has_collection(collection_name): # type: ignore
                    return {"status": f"Collection '{collection_name}' already exists in database '{database_name}'."}
                
                is_edge = collection_type.lower() == "edge"
                created_collection = db.create_collection(collection_name, edge=is_edge) # type: ignore
                return {"status": f"Collection '{collection_name}' (type: {'edge' if is_edge else 'document'}) created successfully in database '{database_name}'.", "collection_info": created_collection.properties()}
                ...
                return {"status": f"Collection '{collection_name}' deleted successfully from database '{database_name}'."}
        ...
        except (CollectionListError, CollectionCreateError, CollectionDeleteError, CollectionPropertiesError, CollectionConfigureError, ArangoServerError) as e:
            logger.error(f"CollectionManagementAgent: ArangoDB error in DB '{database_name}', Collection '{collection_name}' - Op '{operation}': {e}")
            return {"error": f"ArangoDB Collection Error: {e.error_message if hasattr(e, 'error_message') else str(e)}", "error_code": e.error_code if hasattr(e, 'error_code') else None}
        except Exception as e:
            logger.error(f"CollectionManagementAgent: Unexpected error in DB '{database_name}', Collection '{collection_name}' - Op '{operation}': {e}", exc_info=True)
