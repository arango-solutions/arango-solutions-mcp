...
from arango.database import StandardDatabase  # Import StandardDatabase
from arango.exceptions import ArangoServerError  # Import ArangoServerError
from arango.exceptions import (DatabaseCreateError, DatabaseDeleteError,
                               DatabaseListError)

from ..arango_connector import \
    arango_connector  # arango_connector.get_db() returns StandardDatabase
    ...
    async def arun(self, mcp_tool_inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if operation == "list_databases":
                ...
                return {
                    "status": f"Database '{db_to_create_name}' created.",
                    "success": success,
                }
                ...
                success = system_db.delete_database(
                    db_to_delete_name, ignore_missing=False
                )
                return {
                    "status": f"Database '{db_to_delete_name}' deleted.",
                    "success": success,
                }
                ...
                if (
                    not arango_connector.client
                ):  # Should be caught above, but for safety
                    return {"error": "ArangoDB client not initialized."}

                db_to_inspect: StandardDatabase = arango_connector.client.db(
                    target_db_name,
                    username=settings.arango.root_username,
                    password=settings.arango.root_password,
                )
