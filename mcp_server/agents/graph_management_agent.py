...
from arango.exceptions import (ArangoServerError, GraphCreateError,
                               GraphDeleteError, GraphListError)
...
class GraphManagementAgent(ArangoAgentBase):
    ...
    async def arun(self, mcp_tool_inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # For create_graph
        edge_definitions: Optional[List[Dict[str, Any]]] = mcp_tool_inputs.get(
            "edge_definitions"
        )
        orphan_collections: Optional[List[str]] = mcp_tool_inputs.get(
            "orphan_collections"
        )

        # For create_edge
        edge_collection_name: Optional[str] = mcp_tool_inputs.get(
        ...
        try:
            ...
            return {
                "error": f"ArangoDB Graph Error: {e.error_message if hasattr(e, 'error_message') else str(e)}"
            }
