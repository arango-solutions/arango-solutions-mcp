...
class AQLExecutionAgent(ArangoAgentBase):
    ...
    def _setup_internal_langchain_tools(self) -> List[LangchainBaseTool]:
        return []  # No internal Langchain tools needed for direct AQL execution
    ...
    async def arun(self, mcp_tool_inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Optional: LLM check for query type (read-only vs. modifies-data)
            # For PoC, we assume queries are safe or client is trusted.
            # In a real system, add strict validation or restrictions here.
            # Example using the agent's LLM (self.llm):
            # is_safe_prompt = f"Analyze this AQL query: '{aql_query}'. Is it a read-only query (e.g., FOR...RETURN, no INSERT, UPDATE, REMOVE, REPLACE)? Respond YES or NO."
            # safety_check = await self.llm.ainvoke(is_safe_prompt)
            # if "NO" in safety_check.content.upper():
            #     return {"error": "Query appears to modify data and is not allowed by this tool."}
            ...
            logger.info(
                f"AQLExecutionAgent: Query successful, returned {len(results)} documents."
            )
            return response

        except AQLQueryExecuteError as e:
            logger.error(
                f"AQLExecutionAgent: AQL execution error in DB '{database_name}': {e}"
            )
            return {
            ...
            logger.error(
                f"AQLExecutionAgent: ArangoServerError in DB '{database_name}': {e}"
            )
            return {
