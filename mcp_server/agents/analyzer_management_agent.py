...
from arango.exceptions import (AnalyzerCreateError, AnalyzerDeleteError,
                               AnalyzerGetError, AnalyzerListError,
                               ArangoServerError)
        ...
        try:
            ...
            if operation == "list_analyzers":
                ...
                if not analyzer_name or not analyzer_type:
                    return {
                        "error": "Analyzer name and type are required for creation."
                    }

                # Basic validation, LLM could enhance this
                if (
                    analyzer_type == "ngram"
                ...
                return {
                    "status": "Analyzer created successfully.",
                ...
                if success:  # delete_analyzer returns True/False
                    return {
                        "status": f"Analyzer '{analyzer_name}' deleted successfully."
                    }
                else:
                    return {
            ...
            logger.error(
                f"AnalyzerManagementAgent: Unexpected error - {e}", exc_info=True
            )
