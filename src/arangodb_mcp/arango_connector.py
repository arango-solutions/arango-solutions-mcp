...
from arangodb_mcp.observability.dependencies import run_arango_inline, run_arango_sync
from arangodb_mcp.policy.actor_context import get_request_identity

logger = logging.getLogger(__name__)


class DatabaseAccessDenied(PermissionError):
    """Raised when a request targets a database outside effective authority."""
...
class ArangoDBConnector:
    ...
    async def connect(self) -> None:
        ...
        for attempt in range(max_retries + 1):
            try:
                await run_arango_sync(self._connect_sync)
                return
    ...
    def get_db(self, db_name: Optional[str] = None) -> StandardDatabase:
        ...
        if identity is not None:
            if database_name not in identity.databases:
                raise DatabaseAccessDenied(
                    f"Actor {identity.actor_id!r} is not authorized for database {database_name!r}"
                )
            username = identity.credentials.username
            password = identity.credentials.password
        else:
            # Stdio and the legacy static bearer mode deliberately retain their
            # historical configured database identity.
            username = settings.arango.root_username
            password = settings.arango.root_password.get_secret_value()
    ...
    def health_check(self) -> bool:
        ...
        try:
            if not self.client or not self._default_db:
                return False
            run_arango_inline(self._default_db.properties)
            return True
        except Exception as e:
