...
from arangodb_mcp.observability.context import get_request_id, get_traceparent
from arangodb_mcp.observability.conventions import (
    ATTR_CONVENTION_VERSION,
    ATTR_DEPENDENCY_NAME,
    ATTR_OUTCOME,
    ATTR_REQUEST_ID,
    CONVENTION_VERSION,
    SPAN_DEPENDENCY_EMBEDDING,
)
...
async def _post_embedding(
    ...
    *,
    ...
    """Issue one correlated provider call without exposing secrets or input text."""
    headers = {
    ...
    outcome = "error"
    dependency_started("embedding", "embed")
    with get_tracer().start_as_current_span(
        SPAN_DEPENDENCY_EMBEDDING,
        attributes={
            ATTR_CONVENTION_VERSION: CONVENTION_VERSION,
            ATTR_REQUEST_ID: request_id or "",
            ATTR_DEPENDENCY_NAME: "api.openai.com",
            "dependency.operation": "embed",
        },
    ) as span:
        try:
            ...
            dependency_finished("embedding", "embed", outcome, time.perf_counter() - started)
